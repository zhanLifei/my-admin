<template>
	<div class="wrapper">
		...
		<div class="content">
			<h1><a class="title" href="#" target="_blank">vue-cropper</a></h1>
      <canvas id="canvas-compress" width="100" height="100" ></canvas>
			<div class="show-info">
            ...
            <section class="pre-item">
              ...
              <div class="show-preview" :style="{'width': previews.w + 'px', 'height': previews.h + 'px',  'overflow': 'hidden',
				...
				<div class="test-button">
					...
					<label class="btn" for="uploads">上传文件</label>
					...
					<button @click="finish('base64')" class="btn">查看(base64)图</button>
					<button @click="finish('blob')" class="btn">查看(blob)图</button>
...
<script>
  ...
  import { compress,dataurlToCanvas } from '@/ulit/util';
export default {
  data: function() {
    return {
      model: false,
      ...
      option: {
        img: "",
          ...
          // 只有自动截图开启 宽度高度才生效
          autoCropWidth: 130, // 宽度
          autoCropHeight: 160, // 高度
    ...
    finish(type) {
      ...
      if (type === "blob") {
        ...
        this.$refs.cropper.getCropData(data => {
          ...
          this.model = true;
          this.modelSrc = data;
          dataurlToCanvas(data, 'canvas-compress').then(canvas => {
          compress(canvas,50, 1024).then(blob => {
